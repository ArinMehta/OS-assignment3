...
// Adding new system calls that will be used in ps command
extern int sys_getprocscount(void);
extern int sys_getprocid(void);
...
// New system calls for boosted lottery scheduling
extern int sys_settickets(void);
extern int sys_srand(void);
extern int sys_getpinfo(void);
// New system calls to get page table entry and print kernel page table
...
static int (*syscalls[])(void) = {
        ...
        [SYS_mkdir]   sys_mkdir,
        [SYS_close]   sys_close,
        // New system calls that we'll add to the ps command
        [SYS_getprocscount] sys_getprocscount,
        [SYS_getprocid]     sys_getprocid,
...
void syscall(void)
{
    ...
    if((num > 0) && (num <= NELEM(syscalls)) && syscalls[num]) {
        ...
        proc->syscall_count++; // Incrementing the syscall count
        ret = syscalls[num]();
