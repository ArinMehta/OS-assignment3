...
{
    // cprintf("Lottery scheduled at tick: %d\n", ticks);
    if (total_tickets <= 0)
    {
    ...
    uint winner_ticket_number = random_number % total_tickets; // This number is between 0->total_tickets-1
    ...
    for (q = ptable.proc; q < &ptable.proc[NPROC]; q++)
    {
        ...
        int effective_tickets = q->tickets;
        if (q->boostsleft > 0)
        {
            effective_tickets *= 2;
            q->boostsleft--;
        }
        acc += effective_tickets;
        if (acc > winner_ticket_number)
        {
            winner = q;
            break;
        }
...
found:
    p->state = EMBRYO;
    p->pid = nextpid++;
    p->tickets = 1;       // Assign default 1 ticket to the process
    p->syscall_count = 0; // Initialize syscall count to 0
    release(&ptable.lock);
...
int fork(void)
{
    ...
    np->sz = proc->sz;
    np->tickets = proc->tickets; // Inherit tickets from parent process
    np->parent = proc;
    *np->tf = *proc->tf;
...
void exit(void)
{
    ...
    proc->state = ZOMBIE;
    proc->boostsleft = 0; // Reset boostsleft when the process exits
    sched();
...
static void wakeup1(void *chan)
{
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        ...
        {
            // New Implementation
            if (p->duration_of_sleep)
            {
                if (ticks >= p->duration_of_sleep)
                {
                    p->state = RUNNABLE;
                    p->duration_of_sleep = 0; // Set it to zero means no sleep constraint
                    // cprintf("Hello from process %d \n", p->pid); // for testing
                }
            }
            else
            {
...
struct proc *getproc(int n)
{ // returns the pointer to the nth active process; 0 if not found
    struct proc *p;
    int cnt = 0;
...
{ // counts and returns the number of active processes
    struct proc *p;
    int count = 0;
