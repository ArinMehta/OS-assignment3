...
struct run {
    struct run *next;
}; // node of linked list (freelist) of free pages, pointing to next free page

struct {
    struct spinlock lock;
    struct run *freelist; // head of linked list of free pages
} kpt_mem;
...
void paging_init (uint phy_low, uint phy_hi)
{
    PHY_LOW = phy_low;
    PHY_HI = phy_hi;
    kpgdir = P2V(&_kernel_pgtbl);
    mappages (kpgdir, P2V(phy_low), phy_hi - phy_low, phy_low, AP_KU);
    flush_tlb ();
}

// Print valid kernel page table entries (first 10 and last 10)
void kpt(void)
{
    pde_t *pgdir = P2V(&_kernel_pgtbl);
    pde_t pde;
    uint va;

    cprintf("Top 10 PDEs/PTEs at kernel level:\n");
    for (int i = 0; i < 10; i++) {
        va = KERNBASE + i * PDE_SZ + 0xDEF; // offset within a page
        pde = pgdir[PDE_IDX(va)];

        if ((pde & PE_TYPES) == KPDE_TYPE) { // section mapping (1 MB)
            uint pa = (pde & 0xFFF00000) | (va & 0xFFFFF); // section mask
            uint perm = pde & 0xFFF;
            cprintf("va: 0x%x | pde: 0x%x | pa: 0x%x | perm: 0x%x\n", va, pde, pa, perm);
        } else if ((pde & PE_TYPES) == UPDE_TYPE) { // coarse mapping (2nd level page table)
            pte_t *pte = walkpgdir(pgdir, (void*)va, 0);
            if (pte && (*pte & PE_TYPES)) {
                uint pa = PTE_ADDR(*pte) | (va & 0xFFF); // small page mask
                uint perm = *pte & 0xFFF; 
                cprintf("va: 0x%x | pte: 0x%x | pa: 0x%x | perm: 0x%x\n", va, *pte, pa, perm);
            } else {
                cprintf("va: 0x%x | unmapped\n", va);
            }
        } else {
            cprintf("va: 0x%x | unmapped\n", va);
        }
    ...
    cprintf("Last 10 PDEs/PTEs at kernel level:\n");
    uint top_va = KERNBASE + PHY_HI;
    for (int i = 10; i > 0; i--) {
        va = top_va - i * PDE_SZ + 0xDEF; // offset within a page
        pde = pgdir[PDE_IDX(va)];

        if ((pde & PE_TYPES) == KPDE_TYPE) { // section mapping
            uint pa = (pde & 0xFFF00000) | (va & 0xFFFFF);
            uint perm = pde & 0xFFF;
            cprintf("va: 0x%x | pde: 0x%x | pa: 0x%x | perm: 0x%x\n", va, pde, pa, perm);
        } else if ((pde & PE_TYPES) == UPDE_TYPE) { // coarse mapping
            pte_t *pte = walkpgdir(pgdir, (void*)va, 0);
            if (pte && (*pte & PE_TYPES)) {
